...
async function signInform() {
    ...
    // ✅ FIXED: Remove quotes from semail and spassword
    const { data, error } = await supabase.auth.signUp({
        email: semail,        // No quotes!
        password: spassword,  // No quotes!
        options: {
            data: {
                full_name: sname  // Save the name too
            }
        }
    })
    
    // ✅ FIXED: error.message (not massage) and return on error
    if(error) {
    ...
    localStorage.setItem('hasRegistered', 'true')  // Mark user as registered
    alert(sname + " Registered Successfully! Please check your email to verify.")
    ...
    window.location.href = "/login.html"  // Go to login page, not dashboard
}
